...
def sheets(stub):
  ...
  return lambda metadata, data: stub(
      spreadsheets=lambda: stub(
          get=lambda spreadsheetId, fields: stub(
              execute=lambda: metadata),
          values=lambda: stub(
              get=lambda spreadsheetId, range, fields, valueRenderOption: stub(
...
def test_fetch_correct_sheets_and_data(sheets):
  ...
  data = {'Sheet1': {'values': sheet1_vals},
          'Sheet2': {'values': sheet2_vals}}
